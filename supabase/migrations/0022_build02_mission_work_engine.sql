...
-- Tables: uto.missions, uto.mission_proofs, uto.mission_proof_versions,
--         uto.mission_memory_events, uto.capabilities_evidence
-- RPCs:   uto.publish_mission, uto.accept_mission, uto.start_mission,
--         uto.submit_proof, uto.request_changes, uto.resubmit_proof,
--         uto.verify_proof, uto.reject_proof
...
create index if not exists idx_missions_town on uto.missions(town_id);
create index if not exists idx_missions_status on uto.missions(status);
create index if not exists idx_missions_assigned on uto.missions(assigned_to) where assigned_to is not null;

-- ---------- mission proofs (one per mission — the current state) ----------
create table if not exists uto.mission_proofs (
  id                uuid primary key default gen_random_uuid(),
  mission_id        uuid not null references uto.missions(id) on delete cascade,
  ...
  created_at        timestamptz not null default now(),
  updated_at        timestamptz not null default now(),
  constraint mission_proofs_unique_mission unique (mission_id)
);

-- ---------- mission proof versions (immutable history) ----------
create table if not exists uto.mission_proof_versions (
  id                uuid primary key default gen_random_uuid(),
  proof_id          uuid not null references uto.mission_proofs(id) on delete cascade,
  ...
  constraint proof_versions_unique_version unique (proof_id, version_number)
);

create index if not exists idx_proof_versions_proof on uto.mission_proof_versions(proof_id);

-- ---------- mission memory events (append-only, RPC-only writes) ----------
create table if not exists uto.mission_memory_events (
  id                uuid primary key default gen_random_uuid(),
  mission_id        uuid not null references uto.missions(id) on delete cascade,
  event_type        uto.mission_event_type not null,
  actor_id          uuid not null references auth.users(id) on delete cascade,
  town_id           uuid not null references uto.towns(id) on delete cascade,
  proof_id          uuid references uto.mission_proofs(id) on delete set null,
  note              text,
  created_at        timestamptz not null default now()
);

create index if not exists idx_memory_events_mission on uto.mission_memory_events(mission_id);
...
create table if not exists uto.capabilities_evidence (
  ...
  proof_id          uuid references uto.mission_proofs(id) on delete cascade,
  verified_at       timestamptz not null default now(),
  created_at        timestamptz not null default now(),
  constraint capabilities_unique_per_mission unique (mission_id, builder_id)
);

create index if not exists idx_capabilities_builder on uto.capabilities_evidence(builder_id);

-- ===========================================================
-- 04 · UPDATED_AT TRIGGERS
-- ===========================================================

create trigger set_updated_at before update on uto.missions
  for each row execute function public.set_updated_at();

create trigger set_updated_at before update on uto.mission_proofs
  for each row execute function public.set_updated_at();
...
create trigger mission_guard before update on uto.missions
...
create trigger mission_proof_guard before update on uto.mission_proofs
...
alter table uto.missions force row level security;
alter table uto.mission_proofs enable row level security;
alter table uto.mission_proofs force row level security;
alter table uto.mission_proof_versions enable row level security;
alter table uto.mission_proof_versions force row level security;
alter table uto.mission_memory_events enable row level security;
alter table uto.mission_memory_events force row level security;
...
create policy mission_admin_all on uto.missions for all
  using (app.is_national()) with check (app.is_national());

-- Coordinators manage missions in their town
create policy mission_coordinator_manage on uto.missions for all
...
create policy mission_builder_read on uto.missions for select using (
...
-- Admin/ops full access
create policy proof_admin_all on uto.mission_proofs for all
  using (app.is_national()) with check (app.is_national());

-- Coordinators can read/update proofs for missions in their town
create policy proof_coordinator_rw on uto.mission_proofs for all using (
  exists (
    select 1 from uto.missions m
...
create policy proof_builder_read on uto.mission_proofs for select using (
...
create policy proof_builder_insert on uto.mission_proofs for insert
...
-- Admin/ops full access
create policy proof_version_admin_all on uto.mission_proof_versions for all
  using (app.is_national()) with check (app.is_national());

-- Coordinators can read versions for proofs in their town
create policy proof_version_coordinator_read on uto.mission_proof_versions for select using (
  exists (
    select 1 from uto.mission_proofs p
    join uto.missions m on m.id = p.mission_id
...
create policy proof_version_builder_read on uto.mission_proof_versions for select using (
  submitted_by = auth.uid()
);

-- Builders can insert versions for their own proofs (submit_proof/resubmit_proof RPCs)
create policy proof_version_builder_insert on uto.mission_proof_versions for insert
  with check (submitted_by = auth.uid());
...
-- Admin/ops full access
create policy memory_admin_all on uto.mission_memory_events for all
  using (app.is_national()) with check (app.is_national());

-- Authenticated users can read memory events for missions in their town
create policy memory_read on uto.mission_memory_events for select using (
  exists (
    select 1 from uto.role_assignments ra
    where ra.user_id = auth.uid()
...
-- Admin/ops full access
create policy cap_admin_all on uto.capabilities_evidence for all
  using (app.is_national()) with check (app.is_national());

-- Coordinators can read capabilities for builders in their town
create policy cap_coordinator_read on uto.capabilities_evidence for select using (
  exists (
    select 1 from uto.missions m
...
create policy cap_builder_read on uto.capabilities_evidence for select using (
  builder_id = auth.uid()
);
...
create or replace function uto.publish_mission(_mission_id uuid)
...
begin
  ...
  select * into v_mission from uto.missions where id = _mission_id for update;
  if not found then
  ...
  update uto.missions set status = 'open', published_at = now()
  where id = _mission_id and status = 'draft';

  insert into uto.mission_memory_events
...
grant execute on function uto.publish_mission(uuid) to authenticated;
...
create or replace function uto.accept_mission(_mission_id uuid)
returns json language plpgsql security definer set search_path = '' as $$
declare
  v_uid uuid := auth.uid();
  v_mission uto.missions%rowtype;
...
grant execute on function uto.accept_mission(uuid) to authenticated;
...
create or replace function uto.start_mission(_mission_id uuid)
returns json language plpgsql security definer set search_path = '' as $$
declare
  v_uid uuid := auth.uid();
  v_mission uto.missions%rowtype;
begin
  ...
  update uto.missions set status = 'in_progress', started_at = now()
...
grant execute on function uto.start_mission(uuid) to authenticated;
...
create or replace function uto.submit_proof(
  _mission_id        uuid,
  _business_name     text,
...
begin
  ...
  from uto.mission_proofs where mission_id = _mission_id;
  ...
  if v_proof_id is not null then
    ...
    begin
      select status into v_proof_status from uto.mission_proofs where id = v_proof_id;
  ...
  if v_proof_id is null then
    insert into uto.mission_proofs (mission_id, submitted_by, status, current_version)
    values (_mission_id, v_uid, 'pending', 1)
    returning id into v_proof_id;
  else
    update uto.mission_proofs
  ...
  insert into uto.mission_proof_versions
  ...
  update uto.missions set status = 'proof_submitted', updated_at = now()
...
grant execute on function uto.submit_proof(uuid, text, text, text, text, text) to authenticated;
...
create or replace function uto.request_changes(
  _mission_id  uuid,
  _reviewer_note text
) returns json language plpgsql security definer set search_path = '' as $$
declare
  v_uid uuid := auth.uid();
  v_mission uto.missions%rowtype;
  v_proof uto.mission_proofs%rowtype;
begin
  if v_uid is null then
    raise exception 'authentication required' using errcode = '42501';
  end if;

  select * into v_mission from uto.missions where id = _mission_id;
  if not found then
    return json_build_object('error', 'not_found');
  end if;
  ...
  select * into v_proof from uto.mission_proofs where mission_id = _mission_id;
  if not found then
  ...
  update uto.missions set status = 'changes_requested', updated_at = now()
  where id = _mission_id;
...
grant execute on function uto.request_changes(uuid, text) to authenticated;
...
create or replace function uto.verify_proof(_mission_id uuid)
returns json language plpgsql security definer set search_path = '' as $$
declare
  v_uid uuid := auth.uid();
  v_mission uto.missions%rowtype;
...
begin
  ...
  select * into v_proof from uto.mission_proofs where mission_id = _mission_id for update;
  if not found then
    return json_build_object('error', 'no_proof');
  end if;
...
grant execute on function uto.verify_proof(uuid) to authenticated;
...
create or replace function uto.reject_proof(
  _mission_id  uuid,
  _reviewer_note text
) returns json language plpgsql security definer set search_path = '' as $$
declare
...
begin
  ...
  update uto.missions set status = 'rejected', updated_at = now()
  where id = _mission_id;
...
grant execute on function uto.reject_proof(uuid, text) to authenticated;
...
create or replace function uto.get_my_mission(_mission_id uuid)
returns json language plpgsql security definer set search_path = '' as $$
declare
  v_uid uuid := auth.uid();
  v_result json;
begin
  ...
  from uto.missions m
  left join uto.mission_proofs p on p.mission_id = m.id
  left join lateral (
    select json_agg(row_to_json(pv) order by pv.version_number) as arr
    from uto.mission_proof_versions pv where pv.proof_id = p.id
  ) versions on true
...
grant execute on function uto.get_my_mission(uuid) to authenticated;
...
create policy "mission_proof_upload" on storage.objects for insert
  to authenticated
  with check (
...
create policy "mission_proof_read" on storage.objects for select
  to authenticated
  using (
    bucket_id = 'mission-proofs'
    and (
...
create policy "mission_proof_no_delete" on storage.objects for delete
  to authenticated
  using (false);
...
-- DONE
-- ===========================================================
comment on table uto.missions is 'Build 02: Mission work engine. Town-scoped missions with lifecycle.';
comment on table uto.mission_proofs is 'Build 02: Current proof state for a mission. One-to-one with missions.';
comment on table uto.mission_proof_versions is 'Build 02: Immutable proof submission history. Each resubmission increments version.';
comment on table uto.mission_memory_events is 'Build 02: Append-only mission event log. Written only by SECURITY DEFINER RPCs.';
